class expressionArray extends visual_block
{
  constructor(vb={name:'none', x:0, y:0, z:0}){
    vb.type = 'expressionArray'
    super(vb)
    this.args = [];
    this.arg_space = 5
    this.base_tag = null
  ...
  static deserialize(pm, json_obj, afterInit=(e)=>{}){
    var ret = expressionArray.create(pm, json_obj.name, json_obj.x, json_obj.y, ()=>{
      pm.set_serialized_idmap(json_obj.id, ret.id)
      
      async function get_args_and_init(){
        for(var vb of json_obj.args){
          var r = await pm.async_run('deserialize_any', vb) 
          ret.push_argument(r)
        }
  ...
  static deserializeAST(pm, ast, afterInit=(e)=>{}){
    var ret = expressionArray.create(pm, "Array", 100, 100, ()=>{
      async function get_args_and_init(){
  ...
  static create(pm, name, x, y, afterInit=()=>{}){
    var c = new expressionArray({name:name, x:x, y:y})
    c.id = pm.num_obj
    pm.num_obj += 1
